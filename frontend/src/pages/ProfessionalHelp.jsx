import { useState } from "react";
import { useNavigate } from "react-router-dom";
import api from "../api";

export default function ProfessionalHelp() {
    const navigate = useNavigate();

    const [selectedField, setSelectedField] = useState(null);
    const [otherField, setOtherField] = useState("");
    const [experienceLevel, setExperienceLevel] = useState(null);
    const [agree, setAgree] = useState(false);

    // Form States
    const [fullName, setFullName] = useState("");
    const [phone, setPhone] = useState("");
    const [city, setCity] = useState("");
    const [submitted, setSubmitted] = useState(false);

    const fields = [
        "Medical (Doctor / Nurse)",
        "Psychological Support",
        "Legal Consulting",
        "Software Development",
        "Graphic Design",
        "Engineering",
        "Accounting / Finance",
        "Social Work",
        "Logistics & Management",
        "Translation & Languages",
    ];

    const experienceLevels = [
        "Beginner",
        "Intermediate",
        "Advanced",
        "Expert / Specialist",
    ];

    const handleSubmit = async (e) => {
        e.preventDefault();

        if (!selectedField && !otherField) return;
        if (!experienceLevel) return;
        if (!agree) return;

        const field = selectedField || otherField;

        const formData = new FormData();
        formData.append("title", "Professional Help");
        formData.append("donation_type", "skills");
        formData.append(
            "description",
            `Name: ${fullName}
            Phone: ${phone}
            City: ${city}
            Field: ${field}
            Experience: ${experienceLevel}`
        );

        await api.post("/donations/create/", formData);
        setSubmitted(true);
    };


    if (submitted) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-yellow-900 via-slate-900 to-black py-16 px-6 text-white flex items-center justify-center">
                <div className="max-w-xl w-full bg-white/10 backdrop-blur-xl border border-white/20 p-10 rounded-3xl shadow-xl animate-fade-in text-center">

                    <div className="w-20 h-20 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-green-500/30">
                        <span className="text-4xl">✓</span>
                    </div>

                    <h2 className="text-3xl font-bold mb-2">Submission Successful!</h2>
                    <p className="text-gray-300 mb-8">Thank you (Shukran!) for finding ways to help.</p>

                    <div className="bg-white/5 rounded-xl p-6 text-left space-y-3 mb-8 border border-white/10">
                        <h3 className="text-xl font-semibold mb-4 border-b border-white/10 pb-2">Summary</h3>
                        <p><span className="text-gray-400">Type:</span> Professional Assistance</p>
                        <p><span className="text-gray-400">Name:</span> {fullName}</p>
                        <p>
                            <span className="text-gray-400">Field:</span>
                            <span className="text-yellow-400 font-bold ml-1">
                                {selectedField || otherField}
                            </span>
                        </p>
                        <p><span className="text-gray-400">Experience:</span> {experienceLevel}</p>
                        <p><span className="text-gray-400">Phone:</span> {phone}</p>
                        <p><span className="text-gray-400">City:</span> {city}</p>
                    </div>

                    <button
                        onClick={() => navigate("/categorization")}
                        className="w-full bg-green-600 hover:bg-green-700 transition rounded-xl py-3 text-lg font-bold shadow-lg shadow-green-900/20"
                    >
                        OK
                    </button>
                </div>
            </div>
        );
    }

    return (
        <div className="min-h-screen bg-gradient-to-br from-yellow-900 via-slate-900 to-black py-16 px-6 text-white">
            <div className="max-w-3xl mx-auto bg-white/10 backdrop-blur-xl border border-white/20 p-10 rounded-3xl shadow-xl">

                {/* Back */}
                <button
                    onClick={() => window.history.back()}
                    className="mb-6 text-white/70 hover:text-white transition"
                >
                    ← Back
                </button>

                <h1 className="text-4xl font-extrabold text-center mb-6">
                    Professional Help 🛠️
                </h1>

                <p className="text-center text-gray-300 mb-10">
                    Support your community by offering your professional expertise.
                </p>

                <form className="space-y-8" onSubmit={handleSubmit}>

                    {/* Full Name */}
                    <div>
                        <label className="block text-lg mb-2">Full Name</label>
                        <input
                            type="text"
                            required
                            value={fullName}
                            onChange={(e) => setFullName(e.target.value)}
                            className="w-full p-3 rounded-xl bg-white/20 border border-white/30"
                            placeholder="Enter your full name"
                        />
                    </div>

                    {/* Phone */}
                    <div>
                        <label className="block text-lg mb-2">Phone Number</label>
                        <input
                            type="text"
                            required
                            value={phone}
                            onChange={(e) => setPhone(e.target.value)}
                            className="w-full p-3 rounded-xl bg-white/20 border border-white/30"
                            placeholder="07XXXXXXXX"
                        />
                    </div>

                    {/* City */}
                    <div>
                        <label className="block text-lg mb-2">City</label>
                        <input
                            type="text"
                            required
                            value={city}
                            onChange={(e) => setCity(e.target.value)}
                            className="w-full p-3 rounded-xl bg-white/20 border border-white/30"
                            placeholder="Your city"
                        />
                    </div>

                    {/* Professional Field */}
                    <div>
                        <label className="block text-lg mb-3">Select Your Professional Field</label>

                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            {fields.map((field, index) => (
                                <div
                                    key={index}
                                    onClick={() => {
                                        setSelectedField(field);
                                        setOtherField("");
                                    }}
                                    className={`p-4 rounded-xl cursor-pointer text-center border transition backdrop-blur-lg 
                    ${selectedField === field
                                            ? "bg-yellow-600/60 border-yellow-300"
                                            : "bg-white/10 border-white/20 hover:bg-white/20"
                                        }`}
                                >
                                    {field}
                                </div>
                            ))}

                            {/* Other */}
                            <div
                                onClick={() => setSelectedField(null)}
                                className={`p-4 rounded-xl cursor-pointer text-center border transition backdrop-blur-lg
                  ${!selectedField && otherField
                                        ? "bg-yellow-600/60 border-yellow-300"
                                        : "bg-white/10 border-white/20 hover:bg-white/20"
                                    }`}
                            >
                                Other
                            </div>
                        </div>

                        {/* Other Field Input */}
                        {!selectedField && (
                            <input
                                type="text"
                                className="w-full mt-4 p-3 rounded-xl bg-white/20 border border-white/30"
                                placeholder="Enter your field"
                                value={otherField}
                                onChange={(e) => setOtherField(e.target.value)}
                            />
                        )}
                    </div>

                    {/* Experience Level */}
                    <div>
                        <label className="block text-lg mb-3">Experience Level</label>

                        <div className="grid grid-cols-2 gap-4">
                            {experienceLevels.map((level, i) => (
                                <div
                                    key={i}
                                    onClick={() => setExperienceLevel(level)}
                                    className={`p-4 rounded-xl cursor-pointer text-center border transition backdrop-blur-lg 
                    ${experienceLevel === level
                                            ? "bg-yellow-600/60 border-yellow-300"
                                            : "bg-white/10 border-white/20 hover:bg-white/20"
                                        }`}
                                >
                                    {level}
                                </div>
                            ))}
                        </div>
                    </div>

                    {/* Upload Certificate */}
                    <div>
                        <label className="block text-lg mb-2">Upload Experience Certificate (optional)</label>
                        <input
                            type="file"
                            className="w-full p-3 rounded-xl bg-white/10 border border-white/20"
                        />
                    </div>

                    {/* Agreement */}
                    <div className="flex items-start gap-3">
                        <input
                            type="checkbox"
                            checked={agree}
                            onChange={() => setAgree(!agree)}
                            className="w-5 h-5 mt-1"
                        />
                        <p className="text-sm text-gray-300">
                            By confirming this box, you formally acknowledge your commitment to provide professional assistance
                            responsibly, ethically, and in accordance with the organization’s policies.
                        </p>
                    </div>

                    {/* Submit */}
                    <button className="w-full mt-4 bg-yellow-600 hover:bg-yellow-700 transition rounded-xl py-3 text-lg font-bold">
                        Submit Professional Help
                    </button>

                </form>
            </div>
        </div>
    );
}
